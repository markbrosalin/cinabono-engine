import { buildAllApi, buildContextOps, buildPublicApi, genSpanId, genTraceId, } from "../../use-casess/api-core/helpers";
    ...
    _compose(mws, ctx, factory) {
        ...
        const wrapped = ((...payload) => {
            ...
            const dispatch = (i, ...args) => {
                ...
                // Core logic
                if (i === mws.length) {
                    emit("api.useCaseFn.start", { payload: args });
                    const coreFn = factory();
                    const result = coreFn(...args);
                    emit("api.useCaseFn.finish", { result });
                    return result;
                }
                // Middleware layer
                const mw = mws[i];
            ...
            try {
                emit("api.useCase.start", { payload });
                const result = dispatch(0, ...payload);
                emit("api.useCase.finish", { result });
                return result;
            }
            catch (error) {
                emit("api.useCase.error", { error });
                throw error;
            }
        });
        return wrapped;

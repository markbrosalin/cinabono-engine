...
export class IOBuilder {
    constructor(
        private readonly deps: Pick<TemplateBuilderDeps, "getItem" | "getLink" | "getScope">
    ) {}

    public buildInputPins(inputIds: Id[], scope: Scope): CircuitPins<"in", "template"> {
        return this._buildPins({
            itemIds: inputIds,
            scope,
            getItemSafely: (id) => {
                const item = ensureToggle(id, this.deps.getItem);
                if (!item) throw E.item.NotToggle(id);
                return item;
            },
            getPinsCount: (toggle) => {
    ...
    public buildOutputPins(outputIds: Id[], scope: Scope): CircuitPins<"out", "template"> {
        return this._buildPins({
            itemIds: outputIds,
            scope,
            getItemSafely: (id) => {
                const item = ensureLamp(id, this.deps.getItem);
                if (!item) throw E.item.NotLamp(id);
                return item;
            },
            getPinsCount: (lamp) => {
    ...
    private _buildPins<
        TItem extends Item,
        TPin extends CircuitPins<"all", "template">[string],
    >(ctx: {
        itemIds: Id[];
        scope: Scope;
        getItemSafely: (id: Id) => TItem;
        getPinsCount: (el: TItem) => number;
        extractLinks: (itemId: Id, scope: Scope) => [Id, Id[]][];
        mapLink: (linkId: Id) => InnerInOutItem[] | null;
        makePin: (items: InnerInOutItem[]) => TPin;
    }): Record<string, TPin> {
        const pins: Record<string, TPin> = {};
    ...
    private _ensureToggleLinkedToCircuit(link: ItemLink): InnerInOutItem[] | null {
        const item = this.deps.getItem(link.toItemId);
        if (!item) throw E.item.NotFound(link.toItemId);
        return isCircuitItem(item) ? pinOps(item).input.items.get(link.toPin) : null;
    }

    private _ensureLampLinkedToCircuit(link: ItemLink): InnerInOutItem | null {
        const item = this.deps.getItem(link.fromItemId);
        if (!item) throw E.item.NotFound(link.fromItemId);

 ...
 * @returns boolean
 */
type KeyMatchesAnyPattern<Key extends string, Patterns extends readonly string[]> = Patterns extends readonly [infer P extends string, ...infer Rest extends readonly string[]] ? MatchesPattern<Key, P> extends true ? true : KeyMatchesAnyPattern<Key, Rest> : false;
/**
...
export type ExtractSubMapByPatterns<EvMap, Patterns extends readonly string[]> = {
    [K in Keys<EvMap> as K extends string ? KeyMatchesAnyPattern<K, Patterns> extends true ? K : never : never]: EvMap[K];
};
/**
 ...
 */
export type SplitEvent<S extends string> = S extends `${infer A}.${infer B}` ? readonly [A, ...SplitEvent<B>] : readonly [S];
/**
 * Check if an event name matches a pattern with wildcards ("*").
 * @example MatchesPattern<"core.step.start", "core.step.*"> --> true
 * @returns boolean
 */
export type MatchesPattern<Event extends string, Pattern extends string> = CompareSegments<SplitEvent<Event>, SplitEvent<Pattern>>;
/**
